...
function cardTemplate(place, index, mode) {
  const src = SOURCES[place.source];
  const route = ROUTES[place.name] || {difficulty:3, tasks:'待补充', route:'前往方式仍需核验。', reliability:'low', evidence:[place.source]};
  ...
  return `<article class="map-card ${place.featured ? 'featured' : ''}">
    ...
    <div class="card-side">
      <span class="level-badge">Lv.${range[0]}–${range[1]}</span>
...
function render() {
  ...
  const matches = PLACES.filter(p => isMatch(p, level, mode)).sort((a,b) => scorePlace(a,level,mode) - scorePlace(b,level,mode));
  title.textContent = `Lv.${level} ${mode === 'level' ? '练级' : '技能'}推荐`;
...
document.querySelector('#recordCount').textContent = `${PLACES.length} 个地点 · 可离线使用`;
